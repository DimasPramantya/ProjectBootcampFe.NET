...
function populateLoanTable(loans) {
    ...
    loans.forEach(loan => {
        ...
        const isDisabled = loan.status === "repaid" || loan.status === "requested" ? 'disabled' : '';
        tr.innerHTML = `
            <td>${i++}</td>
            <td>${loan.amount}</td>
            <td>${loan.status}</td>
            ...
            <td>
                ...
                <button class="btn btn-danger btn-sm"  onClick="viewLoanRepayment('${loan.id}')" ${isDisabled}>Bayar</button>
            </td>
        `;
...
function viewLoanRepayment(loanId) {
    window.location.href = `/Borrower/RepaidLoan?loanId=${loanId}`;
}
...
function populatePagination(totalRecords, pageNumber, pageSize) {
    ...
    if (pageNumber === totalPages || totalPages === 0) nextPage.classList.add('disabled');
    nextPage.innerHTML = `
